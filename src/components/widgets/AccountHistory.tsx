...
function TransactionIcon({ amount, illustrative }: { amount: number; illustrative: boolean }) {
  return <span aria-label={illustrative ? "Illustrative historical record" : amount < 0 ? "Debit" : "Credit"} className={`grid size-9 shrink-0 place-items-center rounded-full text-base font-bold ${illustrative ? "bg-amber-300/10 text-amber-200" : amount < 0 ? "bg-red-400/10 text-red-200" : "bg-green-400/10 text-green-200"}`}>{illustrative ? "·" : amount < 0 ? "↗" : "↙"}</span>;
}
...
export default function AccountHistory() {
  ...
  const records = useMemo<TimelineRecord[]>(() => [
    ...transactions.map(record => ({ ...record, illustrative: false })),
    ...illustrative,
  ...
  return <section className="bank-surface mt-6 min-w-0 rounded-xl p-4 sm:p-6" aria-label="Transaction history">
    ...
    <p className="mt-3 text-xs text-zinc-500"><span className="mr-1 inline-block size-1.5 rounded-full bg-amber-300 align-middle" />Illustrative history is presentation-only and never changes live balances.</p>
    ...
    <div className="divide-y divide-white/10">
      {filtered.slice(visiblePage * 20, (visiblePage + 1) * 20).map(record => <button type="button" key={record.id} className="grid w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 py-4 text-left transition hover:bg-white/[0.03]" onClick={() => setSelected(record)}>
        <TransactionIcon amount={record.amount} illustrative={record.illustrative} />
        <div className="min-w-0"><h3 className="truncate text-sm font-semibold">{record.name}</h3><p className="mt-1 truncate text-xs text-zinc-400"><time dateTime={record.createdAt}>{record.time}</time> · {record.type}</p></div>
        <p className={`whitespace-nowrap text-sm font-semibold tabular-nums ${record.amount < 0 ? "text-red-300" : "text-green-300"}`}><PrivateAmount value={Math.abs(record.amount)} prefix={record.amount < 0 ? "-$" : "+$"} /></p>
      </button>)}
    ...
    {selected && <aside className={`mt-5 rounded-lg border p-4 ${selected.illustrative ? "border-amber-300/30 bg-amber-300/[0.06]" : "border-green-300/25 bg-green-400/[0.06]"}`} aria-label="Transaction details"><div className="flex items-start justify-between gap-3"><div><p className={`text-xs font-bold uppercase tracking-widest ${selected.illustrative ? "text-amber-200" : "text-green-200"}`}>{selected.illustrative ? "Illustrative record" : "Transaction details"}</p><h3 className="mt-1 font-bold">{selected.name}</h3></div><button type="button" className="text-sm text-zinc-300 underline" onClick={() => setSelected(null)}>Close</button></div><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-zinc-400">Date</dt><dd>{selected.time}</dd></div><div><dt className="text-zinc-400">Reference</dt><dd>{selected.reference || "Not assigned"}</dd></div><div><dt className="text-zinc-400">Method</dt><dd>{selected.method}</dd></div><div><dt className="text-zinc-400">Amount</dt><dd className={selected.amount < 0 ? "text-red-300" : "text-green-300"}><PrivateAmount value={Math.abs(selected.amount)} prefix={selected.amount < 0 ? "-$" : "+$"} /></dd></div></dl>{selected.illustrative && <p className="mt-4 text-xs text-amber-100/75">Presentation data only; it is not a posted transaction.</p>}</aside>}
  </section>;
}
