...
export default function AccountHistory() {
  ...
  return <section className="bank-surface mt-6 min-w-0 rounded-xl p-4 sm:p-6" aria-label="Transaction history">
    ...
    <p className="my-4 text-xs text-zinc-500">{filtered.length} account records · Newest first · Dates in UTC</p>
    <div className="divide-y divide-white/10">
      {filtered.slice(visiblePage * 20, (visiblePage + 1) * 20).map(record => <article key={record.id} className="grid min-w-0 gap-2 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2"><h3 className="break-words text-sm font-semibold">{record.name}</h3><span className="rounded border border-white/15 px-2 py-0.5 text-xs text-zinc-400">{record.status}</span></div>
      ...
      {!filtered.length && <p className="py-8 text-sm text-zinc-400">{historyError ? "History is currently unavailable." : records.length ? "No transactions match these filters." : "No recorded transactions. Historical records will appear here when available."}</p>}
    </div>
