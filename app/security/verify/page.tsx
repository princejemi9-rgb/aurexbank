...
export default function SecurityVerifyPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    const response = await fetch("/api/auth/security-verify", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ passcode }) }).catch(() => null);
    const data = await response?.json().catch(() => null) as { error?: string } | null;
    if (!response?.ok) { setError(data?.error || "Security verification is unavailable. Please try again."); setLoading(false); return; }
