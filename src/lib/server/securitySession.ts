...
export function createSecuritySession(userId: string, token: string, revision: string) {
  ...
  const payload = Buffer.from(JSON.stringify({ userId, token, revision, exp: Math.floor(Date.now() / 1000) + MAX_AGE_SECONDS })).toString("base64url");
...
export function readSecuritySession(cookie: string | undefined): SecuritySession | null {
  ...
  const valid = received.length === Buffer.from(expected).length && timingSafeEqual(received, Buffer.from(expected));
...
export function setSecuritySession(response: NextResponse, value: string) {
  response.cookies.set(SECURITY_SESSION_COOKIE, value, { path: "/", httpOnly: true, sameSite: "strict", secure: process.env.NODE_ENV === "production", maxAge: MAX_AGE_SECONDS });
}

export function clearSecuritySession(response: NextResponse) {
  response.cookies.set(SECURITY_SESSION_COOKIE, "", { path: "/", httpOnly: true, sameSite: "strict", secure: process.env.NODE_ENV === "production", maxAge: 0 });
}
