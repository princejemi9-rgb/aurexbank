...
export function readSecurityPasscode(value: unknown): SecurityPasscodeRecord | null {
  ...
  return typeof record.hash === "string" && typeof record.salt === "string" &&
    typeof record.revision === "string" ? record as SecurityPasscodeRecord : null;
}

function derive(passcode: string, salt: string) {
  return scryptSync(passcode, Buffer.from(salt, "base64url"), 32, { N: SCRYPT_COST, r: 8, p: 1 });
}
...
export function validatePasscodeInput(value: unknown) {
